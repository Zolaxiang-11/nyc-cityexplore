...
async function loadData() {
  const v = Date.now();
  const [siteRes, chartRes] = await Promise.all([
    fetch(`./data/site-content.json?v=${v}`, { cache: "no-store" }),
    fetch(`./data/profile-chart-placeholder.json?v=${v}`, { cache: "no-store" })
  ]);
  return {
...
function renderFinale(finale, chartData) {
  const section = document.createElement("section");
  section.className = "chapter";
  section.id = "chapter-finale";

  const chartCards = chartData.profiles
    .map((profile) => {
      const total = profile.stacks.reduce((sum, item) => sum + item.value, 0);
      const bars = profile.stacks
        .map((item) => {
          const width = (item.value / total) * 100;
          return `<span class="seg-${item.key}" style="width:${width.toFixed(2)}%" title="${item.label}: ${item.value}"></span>`;
        })
        .join("");
      ...
      return `
        <article class="chart-card">
          <h4>${profile.name}</h4>
          <div class="stack" role="img" aria-label="${profile.name} 的四维度堆叠占比">${bars}</div>
        </article>
      `;
    })
    .join("");
  ...
  section.innerHTML = `
    ...
    <div class="finale-chart-area">
      <div class="chart-grid">${chartCards}</div>
      <div class="legend">
        <span class="k-social">社会</span>
        <span class="k-economy">经济</span>
        <span class="k-environment">环境</span>
        <span class="k-transport">交通</span>
      </div>
    </div>
    <article class="text-block">
      <ul class="insight-list">
        ${finale.profiles.map((p) => `<li>${p}</li>`).join("")}
      </ul>
      <p>${finale.closing}</p>
    </article>
  `;
